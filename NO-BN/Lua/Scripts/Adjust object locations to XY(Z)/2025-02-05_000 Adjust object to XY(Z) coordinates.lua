...
function parseSimpleKOF(lines)
	...
	for i, line in ipairs(lines) do
		...
		if line:sub(1,1) ~= "-" then
			...
			if field[1] == "00" then
				writeln("Datablock 00, no data")
			elseif field[1] == "01" then
				enheter.retning = tonumber(line:sub(45,45))			
				enheter.vinkler = tonumber(line:sub(46,46))
				enheter.avstand = tonumber(line:sub(47,47))
				writeln("Datablock 01, changed units to {retning: "..enheter.retning..", vinkler: "..enheter.vinkler..", avstand: "..enheter.avstand.."}")				
			elseif field[1] == "05" then
				local record = {}
				record.key = field[2]:gsub("%s+", "")
				local scale = 1
				if enheter.avstand == 2 then
					scale = 3.280839895 -- scale feet to meters
				end
				...
				if enheter.retning == 1 then
					record.Y = tonumber(field[3])*scale
					record.X = tonumber(field[4])*scale
				elseif enheter.retning == 2 then
					record.X = tonumber(field[3])*scale
					record.Y = tonumber(field[4])*scale
				end			
				record.Z = tonumber(field[5])*scale
				table.insert(result, record)
				writeln("Data block 05, read data as { key: "..record.key..", X: "..record.X..", Y: "..record.Y..", Z: "..record.Z.."}")
			else
				writeln("Data block type "..field[1].." is not implemented. Skipping line "..i)
			end
		else
			writeln("Comment line, no data")
		end
...
write(table.tostring({x=1,y=2}))
local c = 1
--[[
2 kof formater
logging i parsing
feilsjekking kill script
by code, by name, by proximity
]]
--[=[show([[
...
fileFormat = askForKeyword("Select input file type", {"Excel", "KOF 2.0", "Simple KOF"})
...
if fileFormat == "Excel" then
	...
	show(nItems.." rows found in sheet "..sheetName.." in Excel file "..filename)
