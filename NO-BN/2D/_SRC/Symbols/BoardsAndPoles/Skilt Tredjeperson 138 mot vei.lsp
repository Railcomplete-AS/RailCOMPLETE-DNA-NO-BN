...
(defun Skilt_Tredjeperson_138_mot_vei ( / blockName description 
	...
	;
	(setq 
		blockName "NO-BN-2D-SKILT-TREDJEPERSON-138-MOT-VEI"
		description "TRAFIKKSKILT 138 MOT VEI FORAN JERNBANE PLANOVERGANG"
		x 9.0
		; Arms in all four quadrants when centered about origo:
	...
	; Upper 'X' arms:
	(command "._PLINE" p11 p13 p14 p16  p46 p44 p43 p41  p31 p33 p34 p36  p26 p24 p23 p21 _closed_) ; Contour
	(addWipeoutToLastClosedPolyline layer_BoardOrPole_Wipeout _keep_)	
	(command "._PLINE" p12 p13 p14 p15 _closed_)	(drawHatch _filledHatch_)
	(command "._PLINE" p22 p23 p24 p25 _closed_)	(drawHatch _filledHatch_)
	(command "._PLINE" p32 p33 p34 p35 _closed_)	(drawHatch _filledHatch_)
	(command "._PLINE" p42 p43 p44 p45 _closed_)	(drawHatch _filledHatch_)
	(moveUp y1)
	; Epilog
...
(defun Skilt_Tredjeperson_138_mot_vei_Flere_spor ( / blockName description
	...
	;
	(setq 
		blockName "NO-BN-2D-SKILT-TREDJEPERSON-138-MOT-VEI-FLERE-SPOR"
		description "TRAFIKKSKILT 138 MOT VEI FORAN JERNBANE PLANOVERGANG MED FLERE SPOR"
		x 9.0
		; Upper cross arms in all four quadrants when centered about origo:
		x1 (* 0.000 x)  y1 (* 0.064 x)
		x2 (* 0.117 x)  y2 (* 0.131 x)
		x3 (* 0.405 x)  y3 (* 0.298 x)
		x4 (* 0.461 x)  y4 (* 0.202 x)
	...
	(moveUp y1)
	; Lower reversed 'V':
	(command "._PLINE" p51 p53 p54 p56   p66 p64 p63 p61 _closed_) ; Contour
	(addWipeoutToLastClosedPolyline layer_BoardOrPole_Wipeout _keep_)	
	(command "._PLINE" p52 p53 p54 p55 _closed_)	(drawHatch _filledHatch_)
	(command "._PLINE" p62 p63 p64 p65 _closed_)	(drawHatch _filledHatch_)
	(moveUp (- y6a))
	; Epilog
	(addDescriptionBelowOrigo description 1.0)
	(createSchematicBlockFromCurrentGraphics blockName)
