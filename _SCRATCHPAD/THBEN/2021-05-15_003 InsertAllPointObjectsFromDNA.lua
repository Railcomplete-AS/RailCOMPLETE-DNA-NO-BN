...
-- 2021-05-15_003 CLFEY Ask for administration's name (commented out), ask for *.xml file
--
--=========================================================================================================================

function writeln(s) if s then write(s.."\n") else write("\n") end

write("================== Insert all objects from DNA =====================\n")
write("Note: Start a fresh drawing with the DNA in question before running this script.\n")
...
ESC = "\x1C"
repeat 
	adm = askForString("Enter the desired administration abbreviation (format: XX-YY)")
	if adm == ESC then 
		return "*** ABORTED ***\n")
	else
		if adm:match("..%-..") == adm then
			break
		else
			writeln("Please enter a valid railway administration name of the form 'XX-YY'")
		end
	end
until 
--]]
